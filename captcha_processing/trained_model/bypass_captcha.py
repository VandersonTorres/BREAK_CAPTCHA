...
from captcha_processing.model_training.process_captcha import process_images
from utils.helpers import resize_to_fit


def bypass_captcha():
    # Get the trained model and the model_labels
    with open("captcha_processing/trained_model/model_labels.dat", "rb") as translater_file:
        lb = pickle.load(translater_file)

    model = load_model("captcha_processing/trained_model/trained_model.hdf5")
